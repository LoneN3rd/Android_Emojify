...
class Emojifier {
    ...
    static  Bitmap detectFacesAndOverlayEmoji(Context context, Bitmap picture){
        ...
        //Show a Toast if no face is detected
        if (faces.size() == 0){
            Toast.makeText(context, R.string.no_faces, Toast.LENGTH_LONG).show();
        } else{
            ...
            for (int i = 0; i<faces.size(); ++i){
                ...
                // the result of this iteration is an initial image with an emoji embedded on it
                bitmap = addBitmapToFace(bitmap, emojiBitmap, face);
            }
        }
    ...
    //determines the proper Emoji based on the facial expression
    private static Emoji whichEmoji(Face face){
        //log the probability of each eye being open and that the person is smiling
        Timber.d("getClassifications: smilingProb = "+face.getIsSmilingProbability());
        Timber.d("getClassifications: leftEyeProb = "+face.getIsLeftEyeOpenProbability());
        Timber.d("getClassifications: rightEyeProb = "+face.getIsRightEyeOpenProbability());

        boolean smiling = face.getIsSmilingProbability() > SMILING_PROB_THRESHOLD;
        boolean leftEyeClosed = face.getIsLeftEyeOpenProbability() < EYE_OPEN_PROB_THRESHOLD;
        boolean rightEyeClosed = face.getIsRightEyeOpenProbability() < EYE_OPEN_PROB_THRESHOLD;

        //determine appropriate emoji and log
        Emoji emoji;
        if(smiling) {
    ...
    private static Bitmap addBitmapToFace(Bitmap backgroundBitmap, Bitmap emojiBitmap, Face face) {
        ...
        //By default, Bitmaps are immutable, can't be changed
        // Initialize the results bitmap to be a mutable copy of the original image
        Bitmap resultBitmap = Bitmap.createBitmap(backgroundBitmap.getWidth(),
                backgroundBitmap.getHeight(), backgroundBitmap.getConfig());

        // Scale the emoji so it fits properly on the face
        float scaleFactor = EMOJI_SCALE_FACTOR;

        // Google emojis are wider than they are tall
